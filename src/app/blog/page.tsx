...
async function fetchAllPosts(): Promise<BlogPost[]> {
  try {
    ...
    return [...apiPosts, ...staticBlogs.filter(b => !slugSet.has(b.slug))];
  } catch {
    return staticBlogs;
  }
}
    ...
    <div style={{ background: '#fff', color: '#000', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1140, margin: '0 auto', padding: '0 40px' }}>
        ...
        <a href={`/blog/${featured.slug}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
          ...
          >
            {/* Cover */}
            <div style={{ height: 420, background: featured.coverGradient, position: 'relative', overflow: 'hidden' }}>
              <div style={{
          ...
          {rest.map((post, i) => (
            <a
              ...
              style={{
                ...
                borderRight: i < rest.length - 1 ? '1px solid #e8e8e8' : 'none',
                padding: '48px 36px',
              }}
            >
              {/* Cover thumbnail */}
              <div style={{
                height: 200, background: post.coverGradient,
